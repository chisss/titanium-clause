...
[系统全景](#titanium-是什么) · [服务索引](#微服务索引) · [架构边界](#架构与领域边界) · [当前服务](#titanium-clause) · [快速开始](#快速开始)
...
> **条款域**：管理保险条款、责任、告知内容与版本快照，为产品组合和保单合同提供可引用定义。

| 属性 | 内容 |
|---|---|
| 限界上下文 | 条款域（核心域） |
| 核心模型 | InsuranceClause、Coverage、ClauseVersion |
| 主要上游 | Admin |
| 主要下游 | Product、Policy、Claim |
| 默认地址 | [`http://localhost:8083/titanium-clause`](http://localhost:8083/titanium-clause) |
| GitHub | [`titanium-clause`](https://github.com/chisss/titanium-clause) |

### 能力与边界

| 本服务负责 | 本服务不负责 |
|---|---|
| 条款生命周期与版本<br/>保险责任和除外责任<br/>告知书与合同文本模板<br/>条款级结构化规则 | 产品组合与销售计划<br/>最终保费计算<br/>出单后的合同实例 |
...
```mermaid
flowchart LR
    UP[Admin] -->|API / Event| CURRENT[条款域]
    CURRENT -->|API / Event| DOWN[Product、Policy、Claim]
```
...
| 模块 | 职责 |
|---|---|
| `titanium-clause-common` | 通用层：模块内枚举、异常和常量 |
| `titanium-clause-api` | API 层：服务间契约、Feign 接口和 DTO |
| `titanium-clause-domain` | 领域层：聚合、值对象、领域事件及 Port |
| `titanium-clause-application` | 应用层：用例编排、命令与查询协调 |
| `titanium-clause-infrastructure` | 基础设施层：Repository、远程 Adapter、消息与持久化 |
| `titanium-clause-query` | 查询层：CQRS 读模型与查询处理器 |
| `titanium-clause-web` | Web 层：REST 入口、请求校验和响应装配 |
| `titanium-clause-bootstrap` | 启动层：Spring Boot 入口、配置和 Liquibase |
...
```bash
git clone https://github.com/chisss/titanium-clause.git
cd titanium-clause
mvn clean verify
```
...
```bash
mvn -pl titanium-clause-bootstrap -am spring-boot:run
```

默认访问地址为 `http://localhost:8083/titanium-clause`。数据库、Redis、Kafka、下游服务地址及环境变量以 `titanium-clause-bootstrap/src/main/resources/application.yml` 为准。
