...
import com.titanium.clause.dto.ActivateClauseRequest;
import com.titanium.clause.dto.ClauseDTO;
import com.titanium.clause.dto.CreateClauseRequest;
import com.titanium.clause.dto.InactivateClauseRequest;
import com.titanium.clause.dto.UpdateClauseRequest;
import com.titanium.clause.service.ClauseApplicationService;
...
public class ClauseController {
    ...
    public ResponseEntity<ClauseDTO> createClause(@RequestBody CreateClauseRequest request) {
        ...
        ClauseId clauseId = clauseApplicationService.createClause(
                request.getClauseCode(),
                request.getClauseName(),
                request.getClauseType(),
                request.getContent(),
        ...
        // 查询创建后的条款
        var clause = clauseApplicationService.findClause(clauseId.getValue(), tenantId)
    ...
    public ResponseEntity<ClauseDTO> getClauseById(@PathVariable String clauseId) {
        String tenantId = TenantContext.getCurrentTenant();
        var clause = clauseApplicationService.findClause(clauseId, tenantId)
    ...
    public ResponseEntity<ClauseDTO> getClauseByCode(@PathVariable String clauseCode) {
        String tenantId = TenantContext.getCurrentTenant();
        var clause = clauseApplicationService.findClauseByCode(clauseCode, tenantId)
                .orElseThrow(() -> new RuntimeException("条款不存在"));
        return ResponseEntity.ok(toDTO(clause));
    }
    ...
    public ResponseEntity<ClauseDTO> updateClause(@PathVariable String clauseId,
        ...
        String tenantId = TenantContext.getCurrentTenant();
        clauseApplicationService.updateClause(
                clauseId,
                request.getClauseName(),
                request.getClauseType(),
                request.getContent(),
    ...
    public ResponseEntity<List<ClauseDTO>> getClausesByStatus(@PathVariable String status) {
        String tenantId = TenantContext.getCurrentTenant();
        var clauses = clauseApplicationService.findClausesByStatus(status, tenantId);
        var clauseDTOs = clauses.stream()
    ...
    public ResponseEntity<List<ClauseDTO>> getClausesByType(@PathVariable String clauseType) {
        String tenantId = TenantContext.getCurrentTenant();
        var clauses = clauseApplicationService.findClausesByType(clauseType, tenantId);
        var clauseDTOs = clauses.stream()
                .map(this::toDTO)
                .toList();
        return ResponseEntity.ok(clauseDTOs);
    ...
    public ResponseEntity<List<ClauseDTO>> getAllClauses() {
        String tenantId = TenantContext.getCurrentTenant();
        var clauses = clauseApplicationService.findAllClauses(tenantId);
        var clauseDTOs = clauses.stream()
                .map(this::toDTO)
                .toList();
        return ResponseEntity.ok(clauseDTOs);
     ...
     * @return DTO对象
     */
    private ClauseDTO toDTO(com.titanium.clause.aggregate.Clause clause) {
